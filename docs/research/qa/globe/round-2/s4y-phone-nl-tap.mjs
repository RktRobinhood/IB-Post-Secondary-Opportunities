...
R.rest = await state();
await b.shot('s4y-phone-nl-rest-close');
// Real tap on the list entry
const p = await b.evaluate(`(() => { const a = [...document.querySelectorAll('.world__list a')].find(a => /TU Delft/.test(a.textContent)); a.scrollIntoView({ block: 'center', behavior: 'instant' }); const r = a.getBoundingClientRect(); return { x: r.left + r.width / 2, y: r.top + r.height / 2, href: a.href, target: a.target }; })()`);
R.entry = p;
...
await b.shot('s4y-phone-nl-after-tap');
...
await b.shot('s4y-phone-nl-direct');
R.logs = b.logs.filter((l) => !/READ-usage/.test(l)).slice(0, 12);
await fs.writeFile(new URL('./s4y-phone-nl-tap.json', import.meta.url), JSON.stringify(R, null, 1));
