/* Round-2 critic harness: a thin CDP driver against the LIVE site (same approach as ../shoot.mjs, ../round-1/cdp.mjs). */
...
export async function launch({ gpu = true, port = 9360, cpuThrottle = 0 } = {}) {
  const profile = await fs.mkdtemp(path.join(os.tmpdir(), 'globe-crit2-'));
