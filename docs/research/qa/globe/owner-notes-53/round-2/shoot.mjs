/**
 * Round 2 of #53: evidence for each item of critique-round-1.md and the
 * owner's phone notes.
 *
 ...
 *     node docs/research/qa/globe/owner-notes-53/round-2/shoot.mjs
 *
...
const profile = await fs.mkdtemp(path.join(process.env.PROFILE_ROOT || os.tmpdir(), 'shoot53r2-'));
...
const pinsReport = () => evaluate(`(() => {
  ...
  const labels = pins.filter((p) => p.hasAttribute('data-label')).map((p) => p.textContent.trim());
  return {
...
try {
  ...
  await sleep(60);
  await markCard();
  report.phoneUs = { at60ms: await cardState() };
  await shot('03-phone-home-us-60ms', 'Phone home, the United States chosen: 60 ms after the click', { settleMs: 0 });
  await sleep(400);
  report.phoneUs.at460ms = await cardState();
  await shot('03-phone-home-us-460ms', 'Phone home, 460 ms: the same card, mid-flight', { settleMs: 0 });
  await sleep(6000);
  ...
  await g(`const p = g.places.find(p => p.id === 'us'); g.goToPlace(p); return true;`);
  await sleep(6500);
  report.countriesUs = { card: await cardState(), pins: await pinsReport(), alt: await g(`return +g.view.alt.toFixed(3);`) };
