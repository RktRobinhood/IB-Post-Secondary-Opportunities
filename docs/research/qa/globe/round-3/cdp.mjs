/* Round-3 critic harness (copied from round-2): a thin CDP driver against the LIVE site (same approach as ../shoot.mjs, ../round-1/cdp.mjs). */
...
export async function launch({ gpu = true, port = 9360, cpuThrottle = 0 } = {}) {
  const profile = await fs.mkdtemp(path.join(os.tmpdir(), 'globe-crit3-'));
