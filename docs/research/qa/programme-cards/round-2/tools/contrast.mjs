...
const PORT = 9391;
const profile = await fs.mkdtemp('D:/ibp-tmp/pc-con2-');
...
const out = [];
for (const [slug, sel] of [['/', '.card--backdrop'], ['/universities/dk-sdu/', '.card--backdrop'], ['/universities/dk-au/', '.card--backdrop'], ['/universities/dk-via/', '.card--backdrop'], ['/universities/nl-tudelft/', '.card--backdrop']]) for (const scheme of ['light', 'dark']) for (const W of [1280, 390]) {
  await send('Emulation.setDeviceMetricsOverride', { width: W, height: 900, deviceScaleFactor: 1, mobile: W < 500 });
  await send('Emulation.setEmulatedMedia', { features: [{ name: 'prefers-color-scheme', value: scheme }] });
  await send('Page.navigate', { url: `${BASE}${slug}` }); await sleep(2500);
  await ev(`(()=>{const d=document.getElementById('discover-more'); if(d) d.open=true; document.documentElement.style.scrollBehavior='auto'; return 1})()`);
  await ev(`(async()=>{const h=document.documentElement.scrollHeight;for(let y=0;y<h;y+=400){scrollTo(0,y);await new Promise(r=>setTimeout(r,40))}scrollTo(0,0);await Promise.all([...document.querySelectorAll('${sel} img')].map(i=>i.complete?1:new Promise(r=>{i.onload=i.onerror=r;setTimeout(r,8000)})));return 1})()`);
  await sleep(600);
  ...
  const { data } = await send('Page.captureScreenshot', { format: 'png', clip: { x: 0, y: 0, width: W, height: Math.min(H, 16000), scale: 1 }, captureBeyondViewport: true });
  ...
  for (const e of els) {
    ...
    out.push({ page: slug, scheme, W, ...e, min: +vals[0].toFixed(2), p5: +vals[Math.floor(vals.length * 0.05)].toFixed(2), med: +vals[Math.floor(vals.length / 2)].toFixed(2) });
  }
  console.log(slug, scheme, W, els.length); await ev(`document.getElementById('__hide')?.remove()`);
}
await fs.writeFile(path.join(import.meta.dirname, '../contrast.json'), JSON.stringify(out, null, 1));
const byCls = {};
for (const o of out) { const k = `${o.scheme} ${o.W} ${String(o.cls).split(' ')[0]} ${o.color} ${o.fs}/${o.fw}`; (byCls[k] ||= []).push(o); }
