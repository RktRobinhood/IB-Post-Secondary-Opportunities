/* The flat world window — the fallback since ADR 0005.
 *
 * This was the whole interaction layer until the globe (assets/js/globe.js)
 * replaced it. It now runs only where the globe cannot: no WebGL, a context the
 * browser refused or lost, or `?map=flat` in the address. `map.js` decides
 * which one a figure gets; this file is unchanged in behaviour otherwise, so
 * the fallback is the map that shipped and was tested, not a new one nobody
 * has looked at.
 *
 * What follows is its original account of itself.
 */
 ...
 */
export function enhanceFlat(figure) {
  ...
  function choose(id) {
    ...
    if (figure.dispatchEvent(ev) && p.href) follow(p.href);
  }
