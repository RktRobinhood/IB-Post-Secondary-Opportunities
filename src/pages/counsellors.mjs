...
import { hero, note, crumbs, dataTable, stamp, facts, topic } from '../lib/components.mjs';
 ...
 *
 * Shape (docs/research/ia/text-walls.md §3.8): the coverage table leads; the
 * conversation order is a four-step strip; the four things students get wrong
 * are four cards, one line each with the evidence a tap down; the rest are
 * short answers with the full text beneath. Counsellors read more than
 * students, but they scan too.
 */
  ...
  <div class="wrap">
    ...
    <div class="layout-aside">
      <div class="prose">
        ...
        <p>From what cannot be changed to what can.</p>
        <ol class="cards conv-strip">
          <li><strong>Subjects</strong><span>Levels close doors years early. <a href="${url('/planner/')}">Subject checker</a></span></li>
          <li><strong>Country</strong><span>Recognition, deadlines, cost. Most decisions settle here.</span></li>
          <li><strong>Programme</strong><span>The requirements, in the official wording.</span></li>
          <li><strong>Institution</strong><span>Its own page, always, before anything is submitted.</span></li>
        </ol>
        <details class="topic__more">
        ...
        <ul class="cards pitfalls">
          <li>
            <strong>They think grades are the obstacle.</strong>
            <p>Far more often it is a subject level: Mathematics HL, not a better grade.</p>
            <details><summary>An example</summary>
              <p>Mathematics at A level in Danish terms is HL in IB terms. A student with excellent grades and
              Mathematics SL is shut out of engineering everywhere in Denmark, and nothing about their grades
              fixes it.</p></details>
          </li>
          <li>
            <strong>They think CAS and the Extended Essay count.</strong>
            <p>No admissions office in Denmark, Norway, Sweden, Germany or the Netherlands will read either.</p>
            <details><summary>What that means</summary>
              <p>They are worth doing and they are not admissions instruments, and a student spending anxious months
              optimising them for university is spending them for nothing.</p></details>
          </li>
          <li>
            <strong>They think the deadline is the end of the day.</strong>
            <p>The Danish deadline is 15 March at <em>twelve noon</em>, not midnight. The Dutch numerus fixus deadline is 15 January.</p>
          </li>
          <li>
            <strong>They forget results arrive after the deadlines.</strong>
            <p>IB results come out on 6 July; most European deadlines fall months earlier.</p>
            <details><summary>How each system handles it</summary>
              <p>Every system has a different answer to that — Denmark has the IB results service, the Netherlands
              decides it per institution. It is administrative rather than academic, and it is the most common way a
              good application fails.</p></details>
          </li>
        </ul>

        ${topic({
          id: 'check',
          title: 'What to check before repeating something from here',
          short: 'Three things, all on the page itself: the check date, whether a date is marked provisional, and the source link.',
          body: html`<ul class="ticks">
        ...
        ${topic({
          id: 'verified',
          title: 'How much of this has a person checked',
          short: `**${ev.verified || 0} of ${ev.total || 0}** evidence records signed off by a person; ${ev.sourceChecked || 0} re-read automatically against their source.`,
          body: html`<p>The honest answer, which the site publishes on every build rather than on request.</p>
            ${facts([
              {
          ...
          more: 'What the two numbers mean',
        })}

        ${topic({
          id: 'not',
          title: 'What it will not do, by design',
          short: 'No chances, no rankings, no submissions, and no guesses: a figure nobody could verify is left blank.',
        ...
        ${topic({
          ...
          short: html`<p>A wrong deadline costs a student a year. ${site.config?.corrections?.issuesUrl
            ? html`<a href="${site.config.corrections.issuesUrl}" rel="noopener">Open an issue</a>: a claim that could cause harm is pulled within one working day, a deadline or entry requirement checked within three.`
            : 'Tell us: a claim that could cause harm is pulled within one working day, a deadline or entry requirement checked within three.'}</p>`,
        })}
      </div>
